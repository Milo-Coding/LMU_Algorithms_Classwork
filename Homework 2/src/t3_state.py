...
class T3State:
    ...
    def get_transitions(self) -> Iterator[tuple["T3Action", "T3State"]]:
        ...
        """
        # get all open tiles
        open_tiles: list = self.get_open_tiles()

        # get each avalible move at each free space
        for tile in open_tiles:
            moves: list = self.get_moves()
            for move in moves:
                # create a T3Action with the given values
                myAct: T3Action = T3Action(tile[0], tile[1], move)

                # if that action is valid, have the generator yeild it
                if self.is_valid_action(myAct):
                    yield (myAct, self.get_next_state(myAct))
