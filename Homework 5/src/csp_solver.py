    ...
    Returns:
        Optional[list[datetime]]:
            ...
            If no solution is possible:
                Returns None
    '''
    # set up domains for each meeting
    domains: list[set[datetime]] = []
    for _ in range(n_meetings):
        domains.append(date_range.copy())

    # node consistancy can be done before starting recursion
    node_consistency(domains, constraints)

    # start the recursive function
    return recursive_backtracker (list(), n_meetings, domains, constraints)

def recursive_backtracker (assignment: list, variables: int, domains: list[set[datetime]], constraints: set[DateConstraint]) -> Optional[list[datetime]]:
    # get current index
    index: int = len(assignment)

    # base case
    # if all assignments have been made and are valid
    if index == variables and constraints_are_valid(assignment, constraints):
        return assignment
    
    # apply arc consistency before checking each value
    arc_consistency(domains, constraints)

    # recursive case
    # check all datetimes in the domain of the meeting we are assigning
    for time in domains[index].copy():
        assignment.append(time)
        # check if new variable is valid
        if constraints_are_valid(assignment, constraints):
            # if so, move on to check next var
            result = recursive_backtracker(assignment, variables, domains, constraints)
            # if result is not a failure, return it
            if result != None:
                return result
        # if we get here that assignment is invalid so remove it
        assignment.pop()

    # If we get here, ALL values in the domain of next_var failed, so backtrack:
    return None

def constraints_are_valid(assignment: list, constraints: set[DateConstraint]) -> bool:
    # check each constraint
    for constraint in constraints:
        # return false if not valid assignment
...
def node_consistency(domains: list[set[datetime]], constraints: set[DateConstraint]) -> None:
    ...
    '''
    # track index to see which domain we are checking
    index = 0
    for domain in domains:
        for constraint in constraints:
            # skip binary constrainsts
            if constraint.arity() == 2:
                continue

            # compare each date with each constraint
            # create a copy of the domain so we can pare down domains as we go
            for date in domain.copy():
                # remove date if inconsistant
                if constraint.L_VAL == index and not constraint.is_satisfied_by_values(date):
                    domain.remove(date) 
        index += 1
    return
...
def arc_consistency(domains: list[set[datetime]], constraints: set[DateConstraint]) -> None:
    ...
    arc_set: set[Arc] = set()
    for constraint in constraints:
        # skip unary constrainsts
        if constraint.arity() == 1:
            continue

        # add arc in both dircections for each binary constarint
        arc_set.add(Arc(constraint))
        arc_set.add(Arc(constraint.get_reverse()))
    ...
    while arc_set:
        ...
        if remove_inconsistent_values(domains, curr_arc):
            for constraint in constraints:
                ...
                arc = Arc(constraint)
                # if arc points to curr_arc.TAIL, add it back to the arc set
                if arc.HEAD == curr_arc.TAIL:
                    arc_set.add(arc)
                
                # check arc in other direction
                arc = Arc(constraint.get_reverse())
                # if arc points to curr_arc.TAIL, add it back to the arc set
                if arc.HEAD == curr_arc.TAIL:
                    arc_set.add(arc)
    return
...
def remove_inconsistent_values(domains: list[set[datetime]], curr_arc: "Arc") -> bool:
    removed = False
    # for each date in the domain of the current arc's tail
    for tail_date in domains[curr_arc.TAIL].copy():
        # check for consistancy
        satisfied = False
        for head_date in domains[curr_arc.HEAD]:
            if curr_arc.CONSTRAINT.is_satisfied_by_values(tail_date, head_date):
                satisfied = True
        # if no head_date satisfies curr_arc.CONSTRAINT, remove the tail_date
        if not satisfied:
            domains[curr_arc.TAIL].remove(tail_date)
            removed = True
    return removed